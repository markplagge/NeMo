...
typedef struct NeuronModel {
  ...
  tw_stime lastActiveTime;  // last time the neuron fired - used for calculating
                            // leak and reverse functions
  ...
  resetFunDel doReset;
  _neVoltType
      resetVoltParam; /**< Optional parameter for reset voltage function. */

  /**Reverse reset function */
  reverseResetDel reverseReset;

  /** synapse input weights */
  //TODO: ensure that the rando, weight per synapse is ale to be either - or +
  _neVoltType *perSynapseWeight;  // each input synapse has a unique weight or
                                  // probability.
...
//assist functions and functions that have a neuron state in thier headers.
/**
 * @brief neuronReverseFinal final neuron reversal function.
 * USed to roll back any random calls made by the neuron. Reset funs have already
