...
bool TNReceiveMessage(tn_neuron_state *st, messageData *m, tw_lp *lp, tw_bf *bf) {
	...
	switch (m->eventType) {
		...
		case SYNAPSE_OUT:
			st->drawnRandomNumber = tw_rand_integer(lp->rng, 0,
			                                        st->largestRandomValue); //!<- @BUG This might be creating non-deterministic errors
...
void tn_create_neuron(id_type coreID, id_type nID,
                      ...
                      bool kappa, tn_neuron_state *n, int signalDelay, uint64_t destGlobalID, int destAxonID) {
...
void tn_create_neuron_encoded_rv(id_type coreID, id_type nID,
                                 bool synapticConnectivity[NEURONS_IN_CORE],
                                 short G_i[NEURONS_IN_CORE], short sigma[4],
                                 short S[4], bool b[4], bool epsilon,
                                 short sigma_l, short lambda, bool c, uint32_t alpha,
                                 uint32_t beta, short TM, short VR, short sigmaVR, short gamma,
                                 bool kappa, tn_neuron_state *n, int signalDelay, uint64_t destGlobalID,
                                 int destAxonID) {
...
void TN_create_simple_neuron(tn_neuron_state *s, tw_lp *lp) {
	...
	tn_create_neuron_encoded_rv(getCoreFromGID(lp->gid), getNeuronLocalFromGID(lp->gid), synapticConnectivity,
...
//    }
	...
	if (fired && (SAVE_SPIKE_EVTS ||
	              VALIDATION)) { //if we are validating the model or saving spike events, save this event's info.
