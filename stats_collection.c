...
void initDB() {
  ...
  sqlite3_close(db);
  char *cmd =
      sqlite3_mprintf("curl -X PUT http://128.213.23.52:5984/%s", couchName);
  pid_t pID = fork();
  if (pID == 0) {
    exit(system(cmd));
  } else {
...
void writeToCouchTS(char *eventType, int numTextParams, char *txtParamNames[],
  ...
  //	finalStr = sqlite3_mprintf("curl
  //%s%s%s\"final\":\"complete\",\"simTimeStamp\":%d,\"eventType\":\"%s\"%s",cla,data,datav,ts,eventType,endCu);
  //
  // InfluxDB tester:
...
void writeToCouch(char *eventType, int numTextParams, char *txtParamNames[],
  ...
  if (pID == 0) {
    ...
    finalStr = sqlite3_mprintf(
        "curl %s%s%s\"final\":\"complete\",\"eventType\":\"%s\"%s", cla, data,
        datav, eventType, endCu);
    // test string command:
...
void mapRecord(int type, char *typet, int localID, int coreID, int lpid,
  ...
  char *sql =
      sqlite3_mprintf("INSERT INTO mappings (ID, type, typeImp, core, local, "
                      "LPID, gID) VALUES (%i, %Q, %Q, %i, %i, %i, %llu);",
                      val, tpt, typet, coreID, localID, lpid, gid);
  char *zErrMsg = 0;
...
void neuronEventRecord(tw_lpid neuronGlobal, regid_t core, regid_t local,
                       regid_t fromSynapse, tw_stime timestamp, long postPot,
                       char *send) {
  static int row = 0;
  char *zErrMsg = 0;
  char *sql = sqlite3_mprintf(
      "INSERT INTO neuronEvent ( coreID, localID, eventTime, receivedSynapse, "
      "postFirePotential, eventType) VALUES (%i, %i, %f, %i, %i, %Q); ",
      core, local, timestamp, fromSynapse, postPot, send);
  int rc;
  // rc = sqlite3_exec(db, sql, callback, (void*)data, &zErrMsg); //removed, now
  // using couch / influx
  int tp = 1;
  int np = 4;
...
void synapseEventRecord(tw_lpid gid, regid_t core, regid_t local,
                        tw_stime timestamp, tw_lpid dest) {
  static int row = 0;
  char *zErrMsg = 0;
  char *sql = sqlite3_mprintf("insert into SynapseEvents ( \"localID\", "
                              "\"coreID\", \"eventTime\", \"eventID\", "
                              "\"sent\") values (%lu,%lu,%f,%i,%i);",
                              local, core, timestamp, row, dest);
  int rc;
  // rc = sqlite3_exec(db, sql, callback, (void*)data, &zErrMsg); //removed, now
  // using couch / influx.
  row++;
...
void recordNeuron(neuronState *n) {
  ...
  char *zErrMsg = 0;
  char *sql = sqlite3_mprintf(
      "insert into neurons (rowID, neuronID, dendCore,   dendGID, dendLocal, "
      "thresh, other) values (%i, %i, %i, %llu, %i, %i, %i)",
      row, n->neuronID, n->dendriteCore, n->dendriteDest, n->dendriteLocalDest,
  ...
  row++;
  sqlite3_free(sql);
  char *nhds[6] = {"rowID",   "neuronID",  "dendCore",
                   "dendGID", "dendLocal", "thresh"};
  uint64_t tpsv[6] = {row,             n->neuronID,          n->dendriteCore,
