...
def loadFiles(file_list):
	datum = parseFiles(file_list)
	return datum

def workerInDir(directory, ext):
	os.chdir(directory)
	file_list = []
	for file in os.listdir("."):
		if file.endswith(ext):
			file_list.append(file)
	dt = loadFiles(file_list)
	print("worker loaded")
	saveFiles(dt)
			

def saveFiles(paredFiles):
...
if __name__ == '__main__':
	...
	parser.add_argument('outfile_tmplt', nargs='?', help="save to this file", default="dumpi--" + ctimes +
						"-processed-")
	parser.add_argument("--dirname", nargs=1, default="")
	...
	print(arg.dirname)
	if arg.r:
		arg.dirname = arg.dirname[0]
		print("entering dirs with " + arg.dirname + " as prefix")
		dir_list = glob("./" + arg.dirname + "*/")
	elif (len(arg.dirname) > 0):
		arg.dirname = arg.dirname[0]
		ddr = "./" + arg.dirname
		print("entering dir: " + ddr)
		dir_list = glob("./" + arg.dirname + "*/")
	else:
		dir_list = ["./"]
	
	for seldir in dir_list:
		os.chdir(seldir)
		if(arg.all):
		...
		datum = parseFiles(file_list)
		print("saving files in folder parsed ")
		os.mkdir("parsed")
		os.chdir("parsed")
		with concurrent.futures.ThreadPoolExecutor() as executor:
			dats = []
