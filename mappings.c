/*
 * mappings.c -- LP mapping functions were abstracted out of main in order to
...
void initial_mapping_old(void) {
        ...
        for (tw_lpid kpid = 0; kpid < coresPerPe; kpid++) {
                ...
                tw_kp_onpe(kpid, g_tw_pe[0]);  // kp on this pe - each core is a
KP.
                // Now define the neurons/synapses running on this core/kp

                for (int i = 0; i < neuronsInCore; i++) {  // create the neurons
first
                        ...
                        if (DEBUG_MODE)
                                printf("Neuron created on LP %lu, core:local
%lu:%lu, kp is %lu",
