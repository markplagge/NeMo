...
tw_lptype model_lps[] = {
    {
     ...
     (init_f)neuron_init, (pre_run_f)pre_run, (event_f)neuron_event,
     (revent_f)neuron_reverse, (final_f)neuron_final, (map_f)lGidToPE,
     sizeof(neuronState)},
    {

     (init_f)synapse_init, (pre_run_f)pre_run, (event_f)synapse_event,
     (revent_f)synapse_reverse, (final_f)synapse_final, (map_f)lGidToPE,
     sizeof(synapseState)},

    {(init_f)axon_init, (pre_run_f)pre_run, (event_f)axon_event,
     (revent_f)axon_reverse, (final_f)axon_final, (map_f)lGidToPE,
     sizeof(axonState)},
    {0}
...
void neuron_init(neuronState *s, tw_lp *lp) {
  ...
  if (GEN_ON) {  // probabilistic generated mapping
    ...
    s->negativeThreshold =
        tw_rand_integer(lp->rng, NEG_THRESHOLD_MIN, NEG_THRESHOLD_MAX);
    s->resetVoltage =
    ...
    if (stochasticThreshold == true) {
      // random here as well:
      int sizeInBits =
          sizeof(s->thresholdPRNMask) * 8;  //  assuming 8 bits per byte;
    ...
    if (tnMapping == LLINEAR) {
      s->dendriteGlobalDest =
          lGetAxonFromNeu(s->dendriteCore, s->dendriteLocal);
    } else {
...
void synapse_init(synapseState *s, tw_lp *lp) {
  ...
  if (DEBUG_MODE)
    printf(
        ...
        "PE %i , CPE %i\n",
        s->mySynapseNum, lp->gid, s->destNeuron, s->destSynapse, lp->pe->id,lGidToPE(lp->gid));
}
